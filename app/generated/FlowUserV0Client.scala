 ...
 * apibuilder:0.12.20 https://app.apibuilder.io/flow/user/0.3.47/play_2_4_client
 */
...
package io.flow.user.v0 {
  ...
  object Constants {
    ...
    val UserAgent = "apibuilder:0.12.20 https://app.apibuilder.io/flow/user/0.3.47/play_2_4_client"
    val Version = "0.3.47"
    val VersionMajor = 0
    ...
    object EmailVerifications extends EmailVerifications {
      override def postByToken(
        ...
        _executeRequest("POST", s"/users/emails/verifications/${play.utils.UriEncoding.encodePathSegment(token, "UTF-8")}", requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 422 => throw new io.flow.user.v0.errors.GenericErrorResponse(r)
