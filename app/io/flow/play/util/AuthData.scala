...
object AuthDataMap {
  ...
  def fromMap[T <: AuthData](data: Map[String, String])(
    ...
    data.get("created_at").flatMap { ts =>
      val createdAt = new DateTimeFormatterBuilder().appendPattern("[yyyy[-MM[-dd]]]['T'H[:mm[:ss[.SSSXXX]]]['Z']]").parseDefaulting(ChronoField.YEAR_OF_ERA, 1970).parseDefaulting(ChronoField.MONTH_OF_YEAR, 1).parseDefaulting(ChronoField.DAY_OF_MONTH, 1).parseDefaulting(ChronoField.HOUR_OF_DAY, 0).parseDefaulting(ChronoField.MINUTE_OF_HOUR, 0).parseDefaulting(ChronoField.SECOND_OF_MINUTE, 0).parseDefaulting(ChronoField.NANO_OF_SECOND, 0).toFormatter().parse(ts, LocalDateTime.from(_)).toInstant(ZoneOffset.UTC)
