...
/**
  * Generates a new, unqiue, cryptographically secure, unique ID for a resource.
  * Main feature is to prefix the unique id with a three character identifier to
  * understand how the ID is used (e.g. 'F61').
  *
  * Secure id prefixes use the following format:
  *   https://github.com/flowcommerce/standards/blob/primary/Naming.md
